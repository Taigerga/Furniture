    ...
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
        <section aria-label="Upload foto">
          <h2 className="mb-3 text-sm font-medium text-[#334155]">Upload Foto</h2>
        ...
        <section aria-label="Daftar foto">
          <form action="/admin/gallery" method="get" className="mb-3 flex gap-2">
            ...
            <select id="g-f" name="cat" defaultValue={cat} className="rounded-[3px] border border-[#CBD5E1] bg-white px-3 py-2 text-sm">
              ...
              {items.map((g) => (
                <li key={g.id} className="overflow-hidden rounded-[3px] border border-[#E2E8F0] bg-white">
                  ...
                  <form action={updateGalleryAction.bind(null, g.id)} className="space-y-1.5 p-2.5">
                    ...
                    <input id={`gt-${g.id}`} name="title" defaultValue={g.title} className="w-full rounded-[3px] border border-[#E2E8F0] px-2 py-1 text-sm" />
                    <span className="flex gap-1.5">
                      <label className="sr-only" htmlFor={`gc-${g.id}`}>Kategori</label>
                      <select id={`gc-${g.id}`} name="category" defaultValue={g.category} className="flex-1 rounded-[3px] border border-[#E2E8F0] px-2 py-1 text-sm">
                        {CATEGORIES.map((c) => (
                          <option key={c} value={c}>{c}</option>
                        ))}
                      </select>
                      <button type="submit" className="rounded-[3px] border border-[#CBD5E1] px-2.5 py-1 text-xs hover:border-[#0A192F]">
                        Simpan
                      </button>
                    </span>
                  </form>
