    ...
    <div>
      ...
      {items.length === 0 ? (
        ...
        <ul className="space-y-2">
          {items.map((a) => (
            <li key={a.id} className="flex flex-wrap items-center gap-3 rounded-[3px] border border-[#E2E8F0] bg-white p-3">
              <div className="min-w-0 flex-1">
                <p className="truncate font-medium text-[#0F172A]">{a.title}</p>
              ...
              <Link href={`/worker/articles/${a.id}`} className="rounded-[3px] border border-[#CBD5E1] px-3 py-1.5 text-sm hover:border-[#0A192F]">
