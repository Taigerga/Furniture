...
const ArticleEditor = dynamic(() => import("./ArticleEditor").then((m) => m.ArticleEditor), {
  ssr: false,
  loading: () => <div className="rounded-[3px] border border-[#CBD5E1] px-3 py-2 text-sm text-[#64748B]">Memuat editor...</div>,
});
  ...
  return (
    <form action={formAction} className="max-w-3xl space-y-4 rounded-[3px] border border-[#E2E8F0] bg-white p-6">
      ...
      <div>
        ...
        {state?.fieldErrors?.content ? <p role="alert" className="mt-1 text-sm text-[#DC2626]">{state.fieldErrors.content[0]}</p> : null}
      </div>
      <div className="grid gap-4 sm:grid-cols-2">
        <DropzoneInput
          id="ar-thumb"
