...
export default async function WorkerInquiriesPage({
  ...
  // Profil publik, bukan /admin/company-profile (khusus ADMIN).
  ...
  return (
    <div>
      ...
      <p className="mb-4 text-sm text-[#64748B]">Proses inquiry bebas tanpa approval — tercatat atas nama Anda.</p>
      <form action="/worker/inquiries" method="get" className="mb-4 flex flex-wrap gap-2">
        ...
        <select id="ws" name="status" defaultValue={status} className="rounded-[3px] border border-[#CBD5E1] bg-white px-3 py-2 text-sm">
        ...
        <ul className="space-y-2">
          {items.map((inq) => {
            ...
            return (
              ...
              >
                <Link href={`/worker/inquiries/${inq.id}`} className="min-w-0 flex-1">
