    ...
    <div>
      ...
      <div className="mx-auto grid w-full max-w-[97rem] items-start justify-start gap-6 lg:grid-cols-[minmax(0,1fr)_21rem] xl:grid-cols-[minmax(0,52rem)_22rem_20rem]">
        ...
        <aside className="min-w-0 lg:col-span-2 xl:col-span-1" aria-label="Aset dan info tampil">
          ...
          <section className="rounded-[3px] border border-[#E2E8F0] bg-white p-5" aria-label="Aset visual">
            <h2 className="font-medium text-[#0F172A]">Aset Visual</h2>
            <p className="mt-3 text-xs font-medium uppercase tracking-wider text-[#64748B]">Logo</p>
            {company?.logoUrl ? (
              <span className="relative mt-1 block h-20 w-40 overflow-hidden rounded-[3px] border border-[#E2E8F0] bg-[#F4F5F7]">
                <Image src={uploadUrl(company.logoUrl)} alt="Logo perusahaan" fill sizes="160px" className="object-contain" />
              </span>
            ) : (
              <p className="mt-1 rounded-[3px] border border-dashed border-[#E2E8F0] px-3 py-2 text-xs text-[#64748B]">
                Belum ada logo — upload di form.
              </p>
            )}
            <p className="mt-4 text-xs font-medium uppercase tracking-wider text-[#64748B]">Foto hero beranda</p>
            {company?.heroImageUrl ? (
              <span className="relative mt-1 block aspect-[5/4] overflow-hidden rounded-[3px] border border-[#E2E8F0]">
                <Image src={uploadUrl(company.heroImageUrl)} alt="Foto hero beranda" fill sizes="320px" className="object-cover" />
              </span>
            ) : (
              <p className="mt-1 rounded-[3px] border border-dashed border-[#E2E8F0] px-3 py-2 text-xs text-[#64748B]">
