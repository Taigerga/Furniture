...
> **Migrasi berjalan ke NestJS (lihat `../backend/` + `../migrasi.md`):**
> backend NestJS (port 4000) sudah berisi seluruh business logic (auth JWT,
> 12 modul domain, approval workflow + shadow columns 1:1, upload, throttler).
> Frontend sudah punya `lib/api/*` (REST client ke backend) dan `lib/auth.ts`
> yang login via `POST /auth/login` backend. Cara jalan dua server:
> `npm run start:dev` di `../backend` (butuh MySQL + `JWT_SECRET` diisi),
> `npm run dev` di sini (butuh `API_URL=http://localhost:4000`).
> Selama halaman masih memakai `lib/actions/*` + `services/*` lama, `.env`
> lama (`DATABASE_URL`) tetap dibutuhkan. Hapus `lib/actions/`, `services/`,
> `prisma/` dari folder ini HANYA setelah semua halaman pindah ke `lib/api/*`
> dan terverifikasi terhadap backend.
