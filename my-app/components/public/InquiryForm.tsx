...
export function InquiryForm({
  ...
  if (state?.ok) {
    const waLink = whatsapp
      ...
      : undefined;
    return (
      <div className="rounded-[3px] border border-[#BFDBFE] bg-[#DBEAFE] p-6 text-center" role="status">
        <h3 className="text-xl font-bold text-[#0A192F]">Inquiry terkirim</h3>
        <p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-[#334155]">
        ...
        {waLink ? (
          <a
            ...
            className="mt-4 inline-block rounded-[3px] bg-[#0A192F] px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-[#13233F]"
          >
            Lanjut via WhatsApp
          </a>
        ) : null}
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <button
        type="submit"
        disabled={pending}
        className="w-full rounded-[3px] bg-[#0A192F] px-4 py-3 text-sm font-medium text-white transition hover:bg-[#0A192F] active:translate-y-[1px] disabled:opacity-60 sm:w-auto sm:px-8"
      >
