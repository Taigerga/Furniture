  ...
  return (
    <div>
      ...
      {article.approvalStatus === "REJECTED" && article.rejectionReason ? (
        <p role="alert" className="mb-4 rounded-[3px] border border-[#FECACA] bg-[#FEE2E2] px-4 py-3 text-sm text-[#DC2626]">
          Ditolak admin. Alasan: {article.rejectionReason} — perbaiki lalu ajukan ulang.
        </p>
      ) : null}
      {article.thumbnail ? (
        <div className="relative mb-4 h-40 w-full max-w-xl overflow-hidden rounded-[3px] border border-[#E2E8F0]">
          <Image src={uploadUrl(article.thumbnail)} alt="" fill sizes="40vw" className="object-cover" />
        </div>
      ) : null}
      {locked ? (
      ...
      {(article.approvalStatus === "DRAFT" || article.approvalStatus === "REJECTED") && (
        <form action={submitWorkerArticle.bind(null, id)} className="mt-4">
          <button type="submit" className="rounded-[3px] bg-[#0A192F] px-6 py-2.5 text-sm font-medium text-white transition hover:bg-[#0A192F]">
