...
const FIELDS: { key: string; label: string; type?: "text" | "textarea"; hint?: string; inputMode?: "decimal" }[] = [
...
export function CompanyForm({
  ...
  return (
    <form action={formAction} className="space-y-4 rounded-[3px] border border-[#E2E8F0] bg-white p-6">
      <div>
        <h2 className="font-medium text-[#0F172A]">Profil Perusahaan</h2>
      ...
      {FIELDS.map((f) => (
        <div key={f.key}>
          ...
          {f.hint ? (
            <p className="mt-1 text-xs text-[#64748B]">{f.hint}</p>
          ) : null}
