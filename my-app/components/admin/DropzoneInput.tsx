...
export function DropzoneInput({
  ...
  return (
    <div>
      {label ? <span className="mb-1 block text-sm font-medium text-[#334155]">{label}</span> : null}
      <input
        ref={inputRef}
        id={id}
        name={name}
      ...
      <label
        ...
        onDrop={onDrop}
        className={`flex cursor-pointer flex-col items-center justify-center gap-2 rounded-[3px] border-2 border-dashed px-4 py-8 text-center transition ${
          dragging ? "border-[#007BFF] bg-[#DBEAFE]" : "border-[#CBD5E1] bg-[#F4F5F7] hover:border-[#64748B] hover:bg-[#F4F5F7]"
        }`}
      >
        <span className={`flex h-11 w-11 items-center justify-center rounded-[3px] ${dragging ? "bg-[#0A192F] text-white" : "bg-white text-[#64748B] border border-[#E2E8F0]"}`}>
      ...
      {files.length > 0 ? (
        <ul className="mt-2 space-y-1" aria-label="File terpilih">
          {files.map((f, i) => (
            <li key={`${f.name}-${i}`} className="flex items-center justify-between gap-2 rounded-[3px] border border-[#E2E8F0] bg-white px-3 py-1.5 text-sm">
              <span className="min-w-0 truncate text-[#334155]">
                {f.name} <span className="text-[#64748B]">({formatSize(f.size)})</span>
              </span>
              <button
                type="button"
                onClick={() => removeAt(i)}
                aria-label={`Hapus ${f.name}`}
                className="rounded-md p-1 text-[#64748B] transition hover:bg-[#FEE2E2] hover:text-red-600"
              >
