  ...
  return (
    <button
      ...
      className={`rounded-md p-1.5 transition ${
        active ? "bg-[#0A192F] text-white" : "text-[#334155] hover:bg-[#E2E8F0]/70"
      }`}
    >
      {children}
    </button>
...
export function ArticleEditor({ initialHtml = "" }: { initialHtml?: string }) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "min-h-56 max-w-none px-3 py-2 text-sm leading-relaxed text-[#0F172A] outline-none [&_h2]:mt-4 [&_h2]:text-xl [&_h2]:font-semibold [&_h3]:mt-3 [&_h3]:text-lg [&_h3]:font-semibold [&_ul]:list-disc [&_ul]:pl-6 [&_ol]:list-decimal [&_ol]:pl-6 [&_blockquote]:border-l-2 [&_blockquote]:border-[#007BFF] [&_blockquote]:pl-3 [&_blockquote]:italic [&_a]:text-[#0063CE] [&_a]:underline",
      },
    },
  ...
  if (!editor) {
    return <div className="rounded-[3px] border border-[#CBD5E1] px-3 py-2 text-sm text-[#64748B]">Memuat editor...</div>;
  }
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center gap-0.5 rounded-t-lg border border-b-0 border-[#CBD5E1] bg-[#F4F5F7] px-2 py-1.5" role="toolbar" aria-label="Toolbar editor">
      ...
      <div className="rounded-b-lg border border-[#CBD5E1] bg-white focus-within:border-stone-500">
